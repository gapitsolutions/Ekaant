...
- `active_checkins`
- `completed_checkins`
- `items[]` (with patient snapshot: `file_number`, `full_name`, `date_of_birth`, `gender`, `phone`, `patient_category`, `registration_date`)

`registration_date` (the patient's `Patient.registration_date`, ISO `YYYY-MM-DD`) lets the reception dashboard's "Today's Visits" view split check-ins into **New Patients** (registered today) vs **Normal OPD / follow-ups** (registered earlier). It is also present on the custom-range report items (same `_report_patient_payload`).
...
```json
{
  ...
  "entries": [
    {
      "id": "uuid", "date": "2026-06-01", "payment_date": "2026-06-01",
...
payment made. `date` is the **`payment_date`** for payment rows (and the
posting timestamp for invoice/adjustment rows); `payment_date` is `null` for
non-payment rows.

#### `POST /api/v1/pharmacy/suppliers/<supplier_id>/payments/`

Permission: **`IsAdminRole`**. Records a payment (−payable) and refreshes the
cache. Body: `{ amount (>0, required), payment_mode (cash|online|bank),
reference?, note?, payment_date? (YYYY-MM-DD; defaults to today) }`.
`payment_date` is **display/record only** — stored on the ledger row and shown,
but the running balance still follows posting order (`created_at`), so a
back-dated payment cannot rewrite historical balances. Rejected (400) if
`amount` exceeds the current outstanding (no supplier credit). Returns
`{ "outstanding": "<new balance>" }`. The row lock on the supplier serialises
concurrent payments against a fresh balance.
