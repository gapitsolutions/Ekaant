  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(async ([queueData, medicineData, billing]) => {
        ...
        const found =
          queueData.items?.find((item) => item.session_id === sessionId) ||
          null;
        setQueueItem(found);
        setMedicines(medicineData.items || []);
        // Consultation fee comes from admin settings (read-only here). Starts
        // UNSELECTED — staff opt in per dispense via the checkbox below.
        if (billing) {
          const fee = parseFloat(billing.default_consultation_fee) || 0;
          setConfiguredConsultationFee(fee);
          setIncludeConsultationFee(false);
        }
        if (found?.patient_id) {
          try {
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="text-center space-y-3">
          ...
          <p className="text-slate-500 font-bold tracking-tight">
            Loading Dispensing Workstation...
          </p>
        </div>
      </div>
    );
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2.5 mb-1">
                  ...
                  <Badge
                    variant="outline"
                    className="text-[10px] font-bold text-slate-500 bg-slate-50 border-slate-200 uppercase tracking-wider px-2 py-0 h-5 font-mono"
                  >
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="px-5 py-3 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
            <h3 className="font-bold text-slate-800 text-sm flex items-center gap-1.5">
              <Layers className="h-4 w-4 text-primary" /> Formulation &amp;
              Quantity
            </h3>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
              <div className="md:col-span-3 space-y-1.5">
                ...
                <Tabs
                  value={formCategory}
                  onValueChange={(v) =>
                    handleCategoryChange(v as MedicineCategory)
                  }
                >
              ...
              <div className="md:col-span-3 space-y-1.5">
                ...
                {formCategory === "BUP" ? (
                  <Select
                    ...
                    onValueChange={(v) =>
                      handleSubcategoryChange(v as BupStrength)
                    }
                  >
            ...
            <div className="grid grid-cols-2 md:grid-cols-12 gap-4 items-end">
              <div className="md:col-span-3 space-y-1.5">
                ...
                >
                  <SelectTrigger className="h-10 rounded-xl bg-slate-50 border-slate-200 font-bold text-xs">
                    <SelectValue
                      placeholder={selectedMedicine ? "Select Batch" : "—"}
                    />
                  </SelectTrigger>
                  <SelectContent className="rounded-xl border-slate-200 text-xs">
              ...
              <div className="md:col-span-3 space-y-1.5">
                ...
                <div className="h-10 flex items-center px-2">
                  {currentBatch ? (
                    <Badge
                      variant="outline"
                      className="text-xs font-bold text-slate-600 bg-slate-50 border-slate-200 px-2 py-1"
                    >
          ...
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/20">
            <div>
              <h3 className="font-bold text-slate-800 text-sm flex items-center gap-2">
                <ShoppingCart className="h-4 w-4 text-primary" /> Active
                Dispensing List
              </h3>
              <p className="text-xs text-slate-400 mt-0.5">
                Below items are verified and will be deducted from active
                inventory.
              </p>
            </div>
            {lineItems.length > 0 && (
          ...
          {lineItems.length === 0 ? (
            <div className="py-16 text-center">
              ...
              <p className="text-sm text-slate-400 font-semibold">
                Prescription dispensing list is currently empty.
              </p>
              <p className="text-xs text-slate-400 mt-1 max-w-xs mx-auto">
                Select a predefined medicine, enter the total quantity above,
                and add to populate list.
              </p>
            </div>
          ) : (
            <div>
              ...
              <div className="divide-y divide-slate-100">
                {lineItems.map((li) => (
                  <div
                    key={li.id}
                    className="grid grid-cols-12 gap-2 px-5 py-4 items-center hover:bg-slate-50/50 transition-colors"
                  >
                    <div className="col-span-5">
                      <div className="flex items-center gap-1.5">
                        <span className="font-extrabold text-slate-800 text-sm leading-none">
                          {li.medicineName}
                        </span>
                        <Badge
                          variant="outline"
                          className="text-[9px] font-bold py-0.5 px-2 rounded h-4 flex items-center border-slate-200"
                        >
                          {li.category}
                        </Badge>
                      </div>
                    ...
                    <div className="col-span-2 text-center">
                      ...
                      <p className="text-[9px] font-bold text-slate-400">
                        Exp:{" "}
                        {new Date(li.expiryDate).toLocaleDateString("en-IN")}
                      </p>
                    </div>
                    <div className="col-span-2 text-center">
              ...
              <div className="px-5 py-4 border-t border-slate-100 bg-slate-50/30 flex justify-between items-center">
                <span className="text-xs font-extrabold text-slate-500 uppercase tracking-wider">
                  Subtotal ({lineItems.length} Formulation
                ...
                <div className="bg-teal-50/70 border border-teal-100 rounded-xl p-3.5 flex items-center justify-between mt-2">
                  <div className="flex items-center gap-2.5 text-primary">
                    ...
                    <span className="text-xs font-black tracking-tight leading-snug">
                      Next Follow-up:{" "}
                      <span className="underline decoration-wavy decoration-primary/30 ml-0.5">
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-5 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5 min-w-0">
                ...
                <FieldError message={apiErrors.get("payment.payment_method")} />
                <FieldError
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setDiscountRupees(
                        Math.max(
                          0,
                          Math.min(subtotal, parseFloat(e.target.value) || 0),
                        ),
                      )
                    }
                    onWheel={(e) => e.currentTarget.blur()}
              ...
              <div className="space-y-1.5 min-w-0">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    max={totalPayable}
                    value={
                      paymentMethod === "Online" ? onlineAmount : cashAmount
                    }
                    onChange={(e) =>
              ...
              <div className="bg-slate-50/80 border border-slate-100 rounded-xl p-4 space-y-3">
                <div className="flex flex-wrap justify-between items-center gap-2 pb-2 border-b border-slate-200">
                  <span className="text-xs font-extrabold text-slate-500 uppercase tracking-wider flex items-center gap-1">
                    <Activity className="h-3.5 w-3.5 text-primary" /> Split
                    Payment Portions
                  </span>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {[
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        applySplitPreset(preset.cash, preset.online)
                      }
            ...
            <div className="border-t border-slate-100 pt-4 space-y-2.5">
              ...
              >
                <span>
                  {newOutstanding > 0 ? "New Outstanding" : "Fully Settled"}
                </span>
